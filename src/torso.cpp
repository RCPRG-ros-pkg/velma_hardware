...
public:
	...
	void updateHook() {
		...
		// All axes synchronized. Proceed with torque or position control.
			...
			if(++loopCnt > 5000){
				loopCnt = 0;
				testMoveDir = (testMoveDir == 10000) ? (-10000) : 10000;
				mc.setPosition(2, testMoveDir);
				mc.setVelocity(2, 10000);
			}*/
			
			// Command new position values
			if(pos_fs == RTT::NewData){
				std::cout<<"jnt_pos_cmd_ "<<jnt_pos_cmd_(0)<<" "<<jnt_pos_cmd_(1)<<" "<<jnt_pos_cmd_(2)<<" "<<jnt_pos_cmd_(3)<< std::endl;
				mc.setPosition(2, jnt_pos_cmd_(2));
				mc.setVelocity(2, 10000);
				mc.setPosition(3, jnt_pos_cmd_(3));
				mc.setVelocity(3, 10000);
			}
